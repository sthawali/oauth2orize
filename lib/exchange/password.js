...
module.exports = function(options, issue) {
    ...
    return function password(req, res, next) {
        if (!req.body) {
            return next(new Error('OAuth2orize requires body parsing. Did you forget app.use(express.bodyParser())?'));
        }
        ...
        if (!username) {
            return next(new TokenError('Missing required parameter: username', 'invalid_request'));
        }
        if (!passwd) {
            return next(new TokenError('Missing required parameter: password', 'invalid_request'));
        }
        ...
        function issued(err, accessToken, refreshToken, params) {
            ...
            if (!accessToken) {
                return next(new TokenError('Invalid resource owner credentials', 'invalid_grant'));
            }
            ...
            // merge token value with custom response type
            // Send response structure & key for appending token
            if (req.store && req.store.responseStructure && req.store.appendTo) {
                tok = req.store.responseStructure;
                tok[req.store.appendTo].access_token = accessToken;
                if (refreshToken) {
                    tok[req.store.appendTo].refresh_token = refreshToken;
                }
                if (params) {
                    utils.merge(tok[req.store.appendTo], params);
                }
                tok[req.store.appendTo].token_type = tok.token_type || 'Bearer';
            } else {
